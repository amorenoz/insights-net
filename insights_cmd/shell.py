...
from insights_cmd.command import load_commands
...
class Models(dict):
    ...
    def __init__(self, broker, models, cwd, path, cov):
        self._requested = set()
        self._broker = broker
        self._cwd = cwd
        self.path= path
...
def start_shell(paths="",
    ...
    load_packages(parse_plugins(plugins))
    _handle_config(config)

    start_session(paths, False, __coverage=cov)
    if cov:
        cov.stop()
        cov.erase()
